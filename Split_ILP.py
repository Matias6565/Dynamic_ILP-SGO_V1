...
class ILP(object):
	...
	# ================ Relexação 1 ================
	def buildModel_relaxed(self):
		self.mdl = Model()

		#Variáveis de decisão2
		self.x = self.mdl.binary_var_dict([(i,j,w,s) for i in self.rrhs for j in self.nodes for w in self.lambdas for s in self.Split], name = 'Rrh/Node/Lambda/Split: ')
		self.k = self.mdl.binary_var_dict([(w, j, s) for w in self.lambdas for j in self.nodes for s in self.Split], name = 'Lambda/Nonde/Split: ')
		self.y = self.mdl.continuous_var_dict([(i,j) for i in self.rrhs for j in self.nodes], name = 'RRH/Node: ')
		self.xn = self.mdl.binary_var_dict([(j) for j in self.nodes], name = 'Nodes activated: ')
		self.z = self.mdl.binary_var_dict([(w, j) for w in self.lambdas for j in self.nodes], name = 'Lambda/Node: ')
		self.t = self.mdl.continuous_var_dict([(i,j,s) for i in self.rrhs for j in self.nodes for s in self.Split], name = 'RRH/Node/Split: ')#mudei
		self.s = self.mdl.continuous_var_dict([(i,s) for i in self.rrhs for s in self.Split], name = 'Rhh/Split: ')
		self.g = self.mdl.continuous_var_dict([(i,j,w,s) for i in self.rrhs for j in self.nodes for w in self.lambdas for s in self.Split], name = 'Redirections: ')
		self.e = self.mdl.continuous_var_dict([(j) for j in self.nodes], name = "Switch/Node")


		#================ Constraints ================
		self.mdl.add_constraints(self.mdl.sum(self.x[i,j,w,s] for j in self.nodes for w in self.lambdas for s in self.Split) == 2 for i in self.rrhs)
		#self.mdl.add_constraints(self.mdl.sum(self.x[i,j,w,s]) <= 1 for i in self.rrhs for j in self.nodes for w in self.lambdas for s in self.Split)

		self.mdl.add_constraints(self.mdl.sum(self.s[i,s] for s in self.Split) == 1 for i in self.rrhs) # 1 Split por rrh
		self.mdl.add_constraints(self.mdl.sum(self.s[i,s])<=1.0 for s in self.Split for i in self.rrhs)		
		...
		self.mdl.add_constraints(self.mdl.sum(self.y[i,j] for j in self.nodes) == 2 for i in self.rrhs)# 2 nós por rrh
		#self.mdl.add_constraints(self.mdl.sum(self.y[i,j] for j in self.nodes[0:1]) == 1 for i in self.rrhs)
		self.mdl.add_constraints(self.y[i,0] == 1 for i in self.rrhs) # invés do for
		self.mdl.add_constraints(self.y[i,j] <= self.mdl.sum(self.x[i,j,w,s]for s in self.Split for w in self.lambdas) for i in self.rrhs for j in self.nodes)
		self.mdl.add_constraints(Band*self.y[i,j] >= self.mdl.sum(self.x[i,j,w,s]for s in self.Split for w in self.lambdas) for i in self.rrhs for j in self.nodes)
		self.mdl.add_constraints(self.y[i,j] <= self.mdl.sum(self.t[i,j,s] for s in self.Split) for i in self.rrhs for j in self.nodes)
		self.mdl.add_constraints(self.mdl.sum(self.y[i,j])<=1.0 for j in self.nodes for i in self.rrhs)

		self.mdl.add_constraints(self.t[i,j,s] <= self.mdl.sum(self.s[i,s]) for s in self.Split for j in self.nodes for i in self.rrhs)
		self.mdl.add_constraints(self.t[i,j,s] <= self.mdl.sum(self.x[i,j,w,s] for w in self.lambdas) for w in self.lambdas for j in self.nodes for i in self.rrhs for s in self.Split)
		self.mdl.add_constraints(self.mdl.sum(self.t[i,j,s])<=1.0 for j in self.nodes for i in self.rrhs for s in self.Split)


		self.mdl.add_constraints(Band*self.xn[j] >= self.mdl.sum(self.x[i,j,w,s] for i in self.rrhs for w in self.lambdas for s in self.Split) for j in self.nodes)
		self.mdl.add_constraints(self.xn[j] <= self.mdl.sum(self.x[i,j,w,s] for i in self.rrhs for w in self.lambdas for s in self.Split) for j in self.nodes)


		self.mdl.add_constraints(self.mdl.sum(self.z[w,j] for j in self.nodes) <= 1 for w in self.lambdas) # Um lambda por nó
		self.mdl.add_constraints(Band*self.z[w,j] >= self.mdl.sum(self.x[i,j,w,s] for i in self.rrhs for s in self.Split) for w in self.lambdas for j in self.nodes)
		self.mdl.add_constraints(self.z[w,j] <= self.mdl.sum(self.x[i,j,w,s] for i in self.rrhs for s in self.Split) for w in self.lambdas for j in self.nodes)

		self.mdl.add_constraints(self.mdl.sum(self.e[j])<=1.0 for j in self.nodes)		
		self.mdl.add_constraints(self.e[j] <= self.mdl.sum(self.y[i,j] for i in self.rrhs)  for j in self.nodes)
	...
	def solveILP(self):
		...
		#self.sol = self.mdl.round_solution()
		return self.sol
